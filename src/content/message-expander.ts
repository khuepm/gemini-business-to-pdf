...
export class MessageExpander {
  ...
  // Threshold for chunking large conversations
  private readonly CHUNKING_THRESHOLD = 200;
   ...
   * For large conversations (>200 messages), processes in batches using requestIdleCallback
   *
  ...
  async expandAllMessages(): Promise<ExpandResult> {
    ...
    try {
      ...
      // Use chunking for large conversations
      if (collapsedMessages.length > this.CHUNKING_THRESHOLD) {
        Logger.info(`Large conversation detected (${collapsedMessages.length} messages), using chunked processing`);
        await this.expandMessagesInChunks(collapsedMessages, result);
      } else {
        // Process all messages sequentially for smaller conversations
        await this.expandMessagesSequentially(collapsedMessages, result);
      }
  ...
  /**
   * Expand messages sequentially (for smaller conversations)
   *
   * @param messages - Array of message elements to expand
   * @param result - ExpandResult object to update with statistics
   */
  private async expandMessagesSequentially(
  ...
  /**
   * Expand messages in chunks using requestIdleCallback to avoid blocking UI
   *
   * @param messages - Array of message elements to expand
   * @param result - ExpandResult object to update with statistics
   *
   * Requirements: 7.4
   */
  private async expandMessagesInChunks(
    messages: HTMLElement[],
    result: ExpandResult
  ): Promise<void> {
    ...
    Logger.info(`Processing ${messages.length} messages in ${batches.length} batches of ${this.BATCH_SIZE}`);
    ...
    for (let i = 0; i < batches.length; i++) {
      ...
      await this.waitForIdleCallback();
  ...
  /**
   * Wait for browser idle time using requestIdleCallback
   * Falls back to setTimeout if requestIdleCallback is not available
   *
   * @returns Promise that resolves when browser is idle
   */
  private waitForIdleCallback(): Promise<void> {
   ...
   * Disconnects all observers to free memory
   *
   * Validates: Requirements 7.5
   */
  cleanup(): void {
    Logger.info(`Cleaning up ${this.activeObservers.size} active observers`);
    ...
    Logger.info('MessageExpander cleanup complete');
  }
}
